    ...
    function handleHighlight(e) {
        ...
        [...items].forEach((item) => {
            ...
            if (filter === "") {
                ...
                // Tách filter thành các từ
                const filterWords = filter.split(" ");

                // Kiểm tra xem mỗi từ trong filter có xuất hiện trong text không
                const shouldDisplay = filterWords.every((word) => text.includes(word));

                if (shouldDisplay) {
                    // Nếu ít nhất một từ trong filter xuất hiện trong text, hiển thị kết quả mà không ẩn nó
                    item.parentNode.style.display = "block";
                    // Thực hiện highlight cho từng từ trong filter
                    filterWords.forEach((word) => {
                        const highlightedWord = `<span class="primary">${word}</span>`;
