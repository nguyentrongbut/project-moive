...
const itemsSearch = document.querySelectorAll(".header__hotSearch--link");
...
function handleHighlight (e) {
    ...
    [...itemsSearch].forEach((item) => {
        ...
        if (index >=0) {
            ...
            <span class="primary">${text.substring(index, index + filter.length)}</span>
            <span>${text.substring(index, filter.length)}</span>
            `
        }
    });
}