...
window.addEventListener("load", function () {
    ...
    const searchLi = document.querySelectorAll(".header__lists--data li");

    // Kiểm tra xem tất cả các phần tử con có style.display là "none" không
    const allHidden = Array.from(searchLi).every((item) => {
      // Kiểm tra cả trường hợp trống rỗng và "none"
      return item.style.display === "none" || item.style.display === "";
    });
