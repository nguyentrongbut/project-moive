...
window.addEventListener("load", function () {
    ...
    function handleHighlight(e) {
        ...
        [...items].forEach((item) => {
            ...
            // Check if all filter words are included in the item's text
            const isMatch = filterWords.every((word) => text.includes(word));

            if (isMatch) {
                item.parentNode.style.display = "block";
                // Highlight each filter word
                const highlightedText = text.replace(
                    new RegExp(filterWords.join("|"), "ig"),
