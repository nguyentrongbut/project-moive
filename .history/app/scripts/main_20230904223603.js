...
window.addEventListener("load", function () {
 ...
 function handleHighlight(e) {
    ...
    [...items].forEach((item) => {
        ...
        // Sử dụng biểu thức chính quy /g để tìm tất cả các kết hợp khớp trong chuỗi gốc
        const regex = new RegExp(filter, 'ig');
        const newText = text.replace(regex, (matchedText) => {
            return `<span class="primary">${matchedText}</span>`;
        });
