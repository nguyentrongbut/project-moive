...
window.addEventListener("load", function () {
    ...
    function handleHighlight(e) {
        ...
        textedHasSpace = filter.toLowerCase();
        textedNoSpace = filter.toLowerCase().replace(/\s+/g, "");
        [...items].forEach((item) => {
            const text = item.textContent;
            // console.log(text);
            const textHasSpace = text.toLowerCase();
            const textNoSpace = text.toLowerCase().replace(/\s+/g, "");
            console.log(textHasSpace);
            const indexHasSpace = textHasSpace.indexOf(textedHasSpace);
            const indexNoSpace = textNoSpace.indexOf(textedNoSpace);

            if (indexHasSpace >= 0) {
                const matchedText = text.substring(
                    indexHasSpace,
                    indexHasSpace + textHasSpace.length
                );
                const highlightedText = `<span class="primary">${matchedText}</span>`;
                const newText = text.replace(matchedText, highlightedText);
