...
To achieve the functionality where if the user doesn't input spaces but the results have spaces and still display, you can modify the `handleHighlight` function like this:
...
window.addEventListener("load", function () {
    ...
    function handleHighlight(e) {
        let filter = e.target.value.trim(); // Remove leading and trailing spaces
        filter = filter.toLowerCase();

        [...items].forEach((item) => {
            const text = item.textContent.toLowerCase();

            if (text.includes(filter) || text.includes(filter.replace(/\s+/g, ""))) {
                // If filter matches with spaces or without spaces, show the item
...
This code first trims any leading and trailing spaces from the input (`filter`) and then checks if it matches either with spaces or without spaces in the results. If there's a match, it displays the item accordingly.
